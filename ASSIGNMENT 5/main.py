...
# ── Day 6 — Step 24: Search orders by customer name ─────────────

@app.get("/orders/search")
def search_orders(customer_name: str = Query(None)):
    if not customer_name:
        return {"error": "Customer name is required"}
    orders_found = [o for o in orders if customer_name.lower() in o['customer_name'].lower()]
    if not orders_found:
        return {"message": f"No orders found for customer: {customer_name}", "orders": []}
    return {"customer_name": customer_name, "total_found": len(orders_found), "orders": orders_found}



# Search + Sort + Paginate in One Endpoint
...
def browse_products(
    ...
    if order not in ['asc', 'desc']:
        return {'error': "order must be 'asc' or 'desc'"}
    reverse = (order == 'desc')
    sorted_products = sorted(filtered, key=lambda p: p[sort_by], reverse=reverse)
    # Step 3: Paginate
    start = (page - 1) * limit
    end   = start + limit
    paged = sorted_products[start:end]
    return {
        'keyword':     keyword,
        'sort_by':     sort_by,
        'order':       order,
        ...
        'total_pages': -(-len(filtered) // limit),  # ceiling division
        'products':    paged,
    }
