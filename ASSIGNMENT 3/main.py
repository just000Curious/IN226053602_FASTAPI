...
# ── DAY 4 — Step 21: Audit endpoint (GET) ─────────────────────────
...
def audit_products():
    ...
    return {
        ...
        'total_stock_value': total_stock_value * 10,
        "most_expensive": {"name": most_expensive['name'], "price": most_expensive['price']}
    }

# ── BONUS: Category-wide discount endpoint (PUT) ──────────────────
# ⚠️ IMPORTANT: This MUST stay ABOVE all /products/{product_id} routes

@app.put('/products/discount')
def apply_discount(
    category: str = Query(..., description='Category to apply discount to'),
    discount_percent: int = Query(..., gt=0, lt=100, description='Discount percentage'),
):
    affected_products = []
    for product in products:
        if product['category'] == category:
            ...
            product['price'] = max(1, int(original_price - discount_amount))  # Ensure price doesn't go below 1
    ...
    return {
        'message': f'{len(affected_products)} products updated with {discount_percent}% discount',  # Fixed variable name
        'affected_products': affected_products
    }
 
# ── DAY 4 — Step 19: Update stock or price (PUT) ──────────────────
# ⚠️ VARIABLE ROUTES — MUST come AFTER all fixed routes above
